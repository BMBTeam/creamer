...
object InlineServiceLoaderAction : Action<Task> {
    private val INLINE_PATTERN: MethodBodyPattern = MethodBodyPattern(listOf(
    ...
    override fun execute(task: Task) = transformJar(task.outputs.files.singleFile.toPath()) { filer, classNode ->
        if ((classNode.invisibleAnnotations ?: emptyList()).none { it.desc == "Lbmb/kokos/service/InlineService;" })
            return@transformJar // Skip classes without the annotation
